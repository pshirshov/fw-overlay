...
src_install() {
	...
	if use examples; then
		...
		rm -f examples/installall.sh || die 'Can not delete files!'
		rm -f examples/{FastPlayVideoStreaming,LiveVideoStreaming,NativeRTPVideoStreaming,RemoteSharedObjects,SHOUTcast,ServerSideModules,SimpleVideoStreaming,VideoChat,VideoRecording}/{install.bat,install.sh} || die 'Can not delete files!'
		...
		for DIR in FastPlayVideoStreaming LiveVideoStreaming NativeRTPVideoStreaming RemoteSharedObjects SHOUTcast ServerSideModules SimpleVideoStreaming VideoChat VideoRecording; do
			INST="examples/${DIR}/install.sh"

			echo '#!/bin/sh' >> ${INST} || die
			echo "echo 'Installing ${DIR}...'" >> ${INST} || die
			echo "cp -r /usr/share/doc/${PF}/examples/${DIR}/conf/* /etc/${PN}/conf/" >> ${INST} || die
			echo "mkdir /etc/${PN}/applications/bwcheck" >> ${INST} || die
